    ...
    render() {
        ...
        return (
            <>
                ...
                placeHolder = "Please Enter the Name of the Movie You Are Looking For.."
                />
                <Container>
                    <Row>
                        <Col sm = {6} className = "offset-sm-3 text-center">
                            {searchWord ? <h2 >{searchWord} Results found for </h2> : <PageTitle  title = "Popular Movies"/> }
                        </Col>
                    </Row>
                    <Row>
                        {
                ...
                {(currentPage <= totalPage && !loading ) ?
                    <LoadMoreBtn loadMoreMovies={this.loadMoreMovies} text="Show More" currentPage={currentPage} /> : null
                }
            </>
        )
    }
